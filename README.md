...
One other important decision I made was around duplicate-transaction safety. The `transactionId` is generated server-side by this service (`UUID.randomUUID()`) and the client must send an `Idempotency-Key` HTTP header on every create request. A dedicated `IdempotencyService` owns all idempotency logic - it checks the key against an `IdempotencyRepository`, replays the original response on a matching retry (returning `200 OK`), and rejects (with `422`) any retry that reuses the key with a different payload. The `TransactionService` delegates to `IdempotencyService` and only processes a new transaction when the key is unseen. This is the standard idempotency-key pattern used by payment APIs and keeps retries safe from duplicate writes.
